...
class CallViewModel: ObservableObject {
    ...
    func callNumber(_ rawAddress: String) {
        let address = rawAddress.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !address.isEmpty else {
            Log.warn("\(CallViewModel.TAG) Empty number, aborting call")
            return
        }
        ...
        coreContext.doOnCoreQueue { core in
            ...
            if !finalAddress.lowercased().hasPrefix("sip:") {
                finalAddress = "sip:\(finalAddress)"
            }
            
            // FIXED: createAddress now THROWS, NOT OPTIONAL
            let addr: Address
            do {
                addr = try factory.createAddress(addr: finalAddress)
            } catch {
                DispatchQueue.main.async {
                    ToastViewModel.shared.toastMessage = "Invalid phone number"
                    ToastViewModel.shared.displayToast = true
                }
                return
            }
            ...
            do {
                ...
                params.addCustomHeader(headerName: "From", headerValue: address)
                
                // FIXED: inviteAddressWithParams now RETURNS NON-OPTIONAL & THROWS
                let call = try core.inviteAddressWithParams(addr: addr, params: params)
                
                DispatchQueue.main.async {
                    self.telecomManager.outgoingCallStarted = true
                    self.telecomManager.callInProgress = true
                    self.telecomManager.callDisplayed = true
                    self.telecomManager.callStarted = true
                    self.resetCallView()
                }
                
            } catch {
                DispatchQueue.main.async {
                    ToastViewModel.shared.toastMessage = "Failed to start call"
                    ToastViewModel.shared.displayToast = true
                }
            }
        }
